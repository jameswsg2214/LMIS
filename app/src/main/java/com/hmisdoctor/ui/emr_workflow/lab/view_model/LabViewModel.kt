...
import com.hmisdoctor.config.AppConstants.Companion.FAV_TYPE_ID_LAB
...
class LabViewModel(
    ...
    fun getFavourites(
        ...
        apiService?.getFavourites(AppConstants.BEARER_AUTH + userDataStoreBean?.access_token, userDataStoreBean?.uuid!!, departmentID!!, FAV_TYPE_ID_LAB)?.enqueue(RetrofitMainCallback(emrWorkFlowRetrofitCallBack))
        return
    }
    ...
    fun getTemplete(templeteRetrofitCallBack: RetrofitCallback<TempleResponseModel>) {
        ...
        apiService?.getTemplete(AppConstants.BEARER_AUTH + userDataStoreBean?.access_token, userDataStoreBean?.uuid!!, department_UUID!!, FAV_TYPE_ID_LAB)?.enqueue(RetrofitMainCallback(templeteRetrofitCallBack))
        return
    }
    ...
    fun getComplaintSearchResult(facility_id: Int?,
        query: String,
    ...
    fun labInsert(facility_id : Int?,EmrRequestData: EmrRequestModel, configFinalRetrofitCallBack: RetrofitCallback<EmrResponceModel>) {
        if (!Utils.isNetworkConnected(getApplication())) {
            errorText.value = getApplication<Application>().getString(R.string.no_internet)
            return
        }
    ...
    fun getTemplateDetails(templateId: Int?, facilityUuid: Int?, departmentUuid: Int?,getTemplateRetrofitCallback: RetrofitCallback<ResponseLabGetTemplateDetails>) {
        ...
        apiService?.getLastTemplate(
            AppConstants.BEARER_AUTH + userDataStoreBean?.access_token,
            userDataStoreBean?.uuid!!, facilityUuid!!,templateId!!,AppConstants.FAV_TYPE_ID_LAB,department_UUID!!)?.enqueue(RetrofitMainCallback(getTemplateRetrofitCallback))
        return
    }
